...
public class NoticeController {
	...
	public ModelAndView showNotice(@PathVariable("id")Integer id,HttpServletRequest request)throws Exception{
		ModelAndView mav = new ModelAndView();
		Notice notice = noticeService.getNoticeById(id);
		mav.addObject("notice",notice);
		mav.addObject("mainPage","notice/noticeDetails.jsp");
		mav.addObject("navCode",NavUtil.genNavCode("公告信息",request));
	...
	public String list(String title,String page,String rows,HttpServletResponse response)throws Exception{
		...
		long total=noticeService.getNoticeCount(map);
		JsonConfig jsonConfig=new JsonConfig();
		jsonConfig.registerJsonValueProcessor(java.util.Date.class, new DateJsonValueProcessor("yyyy-MM-dd"));
		JSONArray rowss=JSONArray.fromObject(noticeList, jsonConfig);
