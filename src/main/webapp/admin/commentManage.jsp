...
<script type="text/javascript">
	...
	function openCommentReplyDialog(id){
		$.post("${pageContext.request.contextPath}/comment/loadCommentById.do",{commentId:id},function(result){
			$("#id").val(result.id);
	...
	function saveComment(){
		$("#fm").form("submit",{
			url:"${pageContext.request.contextPath}/comment/replay.do",
			onSubmit:function(){
...
<body style="margin: 1px;">
	<table id="dg" title="留言管理" class="easyui-datagrid" fitColumns="true"
		pagination="true" rownumbers="true" url="${pageContext.request.contextPath}/comment/listComment.do"
		fit="true" toolbar="#tb">
		<thead>
			<tr>
			...
			<table cellspacing="8px">
				<tr>
					...
					<td colspan="4"><textarea rows="4" cols="40" id="content"
							name="content" readonly="readonly"></textarea></td>
				</tr>
				<tr>
					<td valign="top">回复内容：</td>
					<td colspan="4"><textarea rows="4" cols="40" id="replyContent"
							name="replyContent" class="easyui-validatebox"
							required="true"></textarea> <input type="hidden" id="id"
						name="id" /> <input type="hidden" id="nickName"
						name="nickName" /> <input type="hidden" id="createTime"
						name="createTime" /></td>
				</tr>
			</table>
		</form>
