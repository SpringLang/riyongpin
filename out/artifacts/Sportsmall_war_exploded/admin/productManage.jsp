...
<head>
    ...
    <script type="text/javascript">
        ...
        $(function () {
            $("#bName").combobox({
                onSelect: function (record) {
                    $("#sName").combobox("reload", "${pageContext.request.contextPath}/productSmallType/comboList.do?bigTypeId=" + record.id);
        ...
        function openProductAddDialog() {
            ...
            url = "${pageContext.request.contextPath}/product/saveorupdate.do";
        }
        ...
        function openProductModifyDialog() {
            ...
            url = "${pageContext.request.contextPath}/product/saveorupdate.do?id=" + row.id;
        }
        ...
        function setProductHot() {
            ...
            $.messager.confirm("系统提示", "您确认要设置这<font color=red>" + selectedRows.length + "</font>个商品为热卖吗？", function (r) {
                if (r) {
                    $.post("${pageContext.request.contextPath}/product/setProductWithHot.do", {ids: ids}, function (result) {
                        if (result.success) {
        ...
        function delProductHot() {
            var selectedRows = $("#dg").datagrid('getSelections');
            if (selectedRows.length == 0) {
                $.messager.alert("系统提示", "请选择要取消热卖的商品！");
                return;
            }
            var strIds = [];
            for (var i = 0; i < selectedRows.length; i++) {
            ...
            var ids = strIds.join(",");
            $.messager.confirm("系统提示", "您确认要取消这<font color=red>" + selectedRows.length + "</font>个商品热卖吗？", function (r) {
                if (r) {
                    $.post("${pageContext.request.contextPath}/product/delProductWithHot.do", {ids: ids}, function (result) {
                        if (result.success) {
                            $.messager.alert("系统提示", "已设置成功！");
                            $("#dg").datagrid("reload");
                        } else {
        ...
        function setProductSpecialPrice() {
            ...
            $.messager.confirm("系统提示", "您确认要设置这<font color=red>" + selectedRows.length + "</font>个商品为特价吗？", function (r) {
                if (r) {
                    $.post("${pageContext.request.contextPath}/product/setProductWithSpecialPrice.do", {ids: ids}, function (result) {
                        if (result.success) {
                            $.messager.alert("系统提示", "已设置成功！");
                            $("#dg").datagrid("reload");
                        } else {
        ...
        function delProductSpecialPrice() {
            var selectedRows = $("#dg").datagrid('getSelections');
            if (selectedRows.length == 0) {
                $.messager.alert("系统提示", "请选择要取消特价的商品！");
                return;
            }
            var strIds = [];
            for (var i = 0; i < selectedRows.length; i++) {
            ...
            var ids = strIds.join(",");
            $.messager.confirm("系统提示", "您确认要取消这<font color=red>" + selectedRows.length + "</font>个商品特价吗？", function (r) {
                if (r) {
                    $.post("${pageContext.request.contextPath}/product/delProductWithSpecialPrice.do", {ids: ids}, function (result) {
                        if (result.success) {
                            $.messager.alert("系统提示", "已设置成功！");
                            $("#dg").datagrid("reload");
                        } else {
...
<table id="dg" title="商品管理" class="easyui-datagrid" fitColumns="true"
       pagination="true" rownumbers="true" url="${pageContext.request.contextPath}/product/adminlist.do"
       fit="true" toolbar="#tb">
    <thead>
    <tr>
...
<div id="tb">
    <div>
        ...
        <a href="javascript:delProductHot()" class="easyui-linkbutton" iconCls="icon-hot" plain="true">取消热卖</a>
        <a href="javascript:delProductSpecialPrice()" class="easyui-linkbutton" iconCls="icon-special" plain="true">取消特价</a>
    </div>
    <div>
        ...
        <table cellspacing="8px">
            ...
            <tr>
                ...
                <td colspan="4"><input type="text" id="price"
                                       name="price" class="easyui-validatebox" required="true"/></td>
            </tr>
            <tr>
                <td>库存：</td>
                <td colspan="4"><input type="text" id="stock"
                                       name="stock" class="easyui-validatebox" required="true"/></td>
            </tr>
            <tr>
                <td>商品图片：</td>
                <td colspan="4"><input type="file" id="pP" name="pPic"/></td>
            </tr>
            <tr>
                <td>所属大类：</td>
                <td colspan="4"><input class="easyui-combobox" id="bName"
                                       name="bigTypeId"
                                       data-options="panelHeight:'auto',editable:false,valueField:'id',textField:'name',url:'${pageContext.request.contextPath}/productBigType/comboList.do'"/>
                </td>
            </tr>
            <tr>
                <td>所属小类：</td>
                <td colspan="4"><input class="easyui-combobox" id="sName"
                                       name="smallTypeId"
                                       data-options="panelHeight:'auto',editable:false,valueField:'id',textField:'name',url:'${pageContext.request.contextPath}/productSmallType/comboList.do'"/>
                </td>
            </tr>
            <tr>
                <td valign="top">备注：</td>
                <td colspan="4"><textarea rows="5" cols="50" id="description"
                                          name="description"></textarea> <input type="hidden"
                                                                                id="proPic" name="proPic"/> <input
                        type="hidden" id="hot"
                        name="hot"/> <input type="hidden" id="hotTime"
                                            name="hotTime"/> <input type="hidden" id="specialPrice"
                                                                    name="specialPrice"/> <input type="hidden"
                                                                                                 id="specialPriceTime"
                                                                                                 name="specialPriceTime"/>
                </td>
            </tr>
        </table>
    </form>
